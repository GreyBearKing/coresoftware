...
void DSTTrackArrayReader::evaluate_track_and_cluster_residual_compression(){
  if(m_track_map){
        std::cout << "m_track_map exists"
                << "\n";
      }
      ...
      if (!(m_track_map&&m_cluster_map&&m_track_array_container_v2)) {
        ...
        //there should only be as many trackContainers as layers
        //std:: cout << "ArrayKeysSize: " << m_container->getArrayKeysSize() << std::endl;
        for(int iTrk = 0; iTrk < ((int)m_track_array_container_v2->size()); iTrk++){
          //auto track = (SvtxTrack_v4*)m_container->arrTrkArr->ConstructedAt(iTrk);
          //unsigned int key = m_container->getArrayKeys(iTrk);
         
          //m_track_map->insert(track);
          std::cout << __FILE__ << "::" << __func__ << "::" << __LINE__ << std::endl;
          trackContainer2 = (SvtxTrackArray_v2*)m_track_array_container_v2->get_trackarray(iTrk);
          
          //now put this information back into original container

          //make a track and store information into it
          SvtxTrack_v4* track = new SvtxTrack_v4();
          std::cout << "track id: " << trackContainer2->get_id() << std::endl;


          std::cout << "track chisq: " << trackContainer2->get_chisq() << std::endl;

          track->set_id(trackContainer2->get_id());
          track->set_vertex_id(trackContainer2->get_vertex_id());
          track->set_positive_charge(trackContainer2->get_positive_charge());
          track->set_chisq(trackContainer2->get_chisq());
          track->set_ndf(trackContainer2->get_ndf());
          track->set_crossing(trackContainer2->get_crossing());

          //insert PCA point, should be included in constructor
          //track->insert_state(new SvtxTrackState_v1(0));

          track->set_x(trackContainer2->get_x());
          track->set_y(trackContainer2->get_y());
          track->set_z(trackContainer2->get_z());
          track->set_px(trackContainer2->get_px());
          track->set_py(trackContainer2->get_py());
          track->set_pz(trackContainer2->get_pz());
          ...
          */
          ...
          std::cout << "Before TPC seeds" << std::endl;
          //set TPC Seed
          if(trackContainer2->get_does_tpc_seed_exist()){
            TPCSeed->set_qOverR(trackContainer2->tpc_seed_get_qOverR());
            TPCSeed->set_X0(trackContainer2->tpc_seed_get_X0());
            TPCSeed->set_Y0(trackContainer2->tpc_seed_get_Y0());
            TPCSeed->set_slope(trackContainer2->tpc_seed_get_slope());
            TPCSeed->set_Z0(trackContainer2->tpc_seed_get_Z0());
            TPCSeed->set_crossing(trackContainer2->tpc_seed_get_crossing());
          ...
          std::cout << "Before silicon seeds" << std::endl;
          //set Silicon Seed
          if(trackContainer2->get_does_silicon_seed_exist()){
            std::cout << "Inside silicon seeds" << std::endl;
            SiliconSeed->set_qOverR(trackContainer2->silicon_seed_get_qOverR());
            SiliconSeed->set_X0(trackContainer2->silicon_seed_get_X0());
            SiliconSeed->set_Y0(trackContainer2->silicon_seed_get_Y0());
            SiliconSeed->set_slope(trackContainer2->silicon_seed_get_slope());
            SiliconSeed->set_Z0(trackContainer2->silicon_seed_get_Z0());
            SiliconSeed->set_crossing(trackContainer2->silicon_seed_get_crossing());
          ...
          //make 2 loops, one over cluster_container, one over compression container
          //loop over silicon+TPOT
          for(int layer = 0; layer < 9; layer++){
            //check if clusterkey is default and if so then skip
            // this is 9999999999999999 in hexidecimal
            //11068046444225730969
            if(!trackContainer2->getValid(layer)){
              std::cout << "No cluster at layer: " << layer << std::endl;
              continue;
            }
            std::cout << "Valid cluster at layer " << layer << ": " << trackContainer2->getValid(layer) << std::endl;
            ...
            if(layer < 7){
              ...
              layerbyte = (uint8_t) layer + (uint8_t) 48;
            }
            //uint8_t layerbyte = (uint8_t) layer;

            std::cout << "Trkr Id: " << unsigned(TrkrId) << std::endl;
            std::cout << "Layer byte: " << unsigned(layerbyte) << std::endl;
            std::cout << "Sector Id: " << unsigned(trackContainer2->getSectorId(layer)) << std::endl;
            std::cout << "Side: " << unsigned(trackContainer2->getSide(layer)) << std::endl;
            

            std::cout << "Generated hitsetkey: " << ((uint32_t) TrkrId << 24) + ((uint32_t) layerbyte << 16 ) + ((uint32_t) trackContainer2->getSectorId(layer) << 8) + trackContainer2->getSide(layer) << std::endl;
            //(TrkrId <<24)+(layerbyte << 16 )+subsurfkey;
            TrkrDefs::hitsetkey hitsetkey = ((uint32_t)TrkrId << 24)+((uint32_t)layerbyte << 16 )+ + ((uint32_t) trackContainer2->getSectorId(layer) << 8) + trackContainer2->getSide(layer);
            if(hitsetkey != TrkrDefs::getHitSetKeyFromClusKey(trackContainer2->getClusKey(layer))){
              std::cout << "Hitsetkey does not match original" << std::endl;
              std::cout << "Original: " << TrkrDefs::getHitSetKeyFromClusKey(trackContainer2->getClusKey(layer)) << std::endl;
              std::cout << "Generated: " << hitsetkey << std::endl;
          ...
          //layer = TrkrDefs::getLayer(cluster_key);
          m_cluster->setLocalX(trackContainer2->getLocalX(layer));
          m_cluster->setLocalY(trackContainer2->getLocalY(layer));
          m_cluster->setSubSurfKey(trackContainer2->getSubSurfKey(layer));
          m_cluster->setPhiError(trackContainer2->getRPhiError(layer));
          m_cluster->setZError(trackContainer2->getZError(layer));
          m_cluster->setAdc(trackContainer2->getAdc(layer));
          m_cluster->setMaxAdc(trackContainer2->getMaxAdc(layer));
          m_cluster->setPhiSize(trackContainer2->getPhiSize(layer));
          m_cluster->setZSize(trackContainer2->getZSize(layer));
          m_cluster->setOverlap(trackContainer2->getOverlap(layer));
          m_cluster->setEdge(trackContainer2->getEdge(layer));
          ...
          for(uint32_t index = 0; index < 1000 && !foundCluster; index++){
            ...
            if(!m_cluster_map->findCluster(clusterkey)){
              ...
              if(layer < 7){
                std::cout << "Inserting Silicon Cluster" << std::endl;
                SiliconSeed->insert_cluster_key(clusterkey);
          ...
          for(int layer = 0; layer < 50; layer++){
            ...
            // this is 9999999999999999 in hexidecimal
            //11068046444225730969
            if(!trackContainer2->getValidResiduals(layer)){
              std::cout << "No cluster at layer: " << layer << std::endl;
              continue;
            }
            std::cout << "Valid cluster at layer " << layer << ": " << trackContainer2->getValidResiduals(layer) << std::endl;
            ...
            //set clusterkey
            uint8_t TrkrId = 2;
            //since this is only tpc

            uint8_t layerbyte = (uint8_t) layer + (uint8_t) 7;
            
            //uint8_t layerbyte = (uint8_t) layer;

            std::cout << "Trkr Id: " << unsigned(TrkrId) << std::endl;
            std::cout << "Layer byte: " << unsigned(layerbyte) << std::endl;
            std::cout << "Sector Id: " << unsigned(trackContainer2->getSectorIdResiduals(layer)) << std::endl;
            std::cout << "Side: " << unsigned(trackContainer2->getSideResiduals(layer)) << std::endl;
            

            std::cout << "Generated hitsetkey: " << ((uint32_t) TrkrId << 24) + ((uint32_t) layerbyte << 16 ) + ((uint32_t) trackContainer2->getSectorIdResiduals(layer) << 8) + trackContainer2->getSideResiduals(layer) << std::endl;
            //(TrkrId <<24)+(layerbyte << 16 )+subsurfkey;
            TrkrDefs::hitsetkey hitsetkey = ((uint32_t)TrkrId << 24)+((uint32_t)layerbyte << 16 )+ + ((uint32_t) trackContainer2->getSectorIdResiduals(layer) << 8) + trackContainer2->getSideResiduals(layer);
          ...
          //m_cluster->setLocalX(trackContainer2->getLocalX(layer));
          //m_cluster->setLocalY(trackContainer2->getLocalY(layer));
          std::cout << "trackContainer subsurfKey: " << trackContainer2->getSubSurfKeyResiduals(layer) << std::endl;
          m_cluster->setSubSurfKey(trackContainer2->getSubSurfKeyResiduals(layer));
          m_cluster->setPhiError(trackContainer2->getRPhiErrorResiduals(layer));
          m_cluster->setZError(trackContainer2->getZErrorResiduals(layer));
          m_cluster->setAdc(trackContainer2->getAdcResiduals(layer));
          m_cluster->setMaxAdc(trackContainer2->getMaxAdcResiduals(layer));
          m_cluster->setPhiSize(trackContainer2->getPhiSizeResiduals(layer));
          m_cluster->setZSize(trackContainer2->getZSizeResiduals(layer));
          m_cluster->setOverlap(trackContainer2->getOverlapResiduals(layer));
          m_cluster->setEdge(trackContainer2->getEdgeResiduals(layer));
          ...
          for(uint32_t index = 0; index < 1000 && !foundCluster; index++){
            ...
            std::cout << "generated clusterkey: " << clusterkey << std::endl;
            if(!m_cluster_map->findCluster(clusterkey)){
              std::cout << "Valid Index: " << index << std::endl;
              std::cout << "Valid generated clusterkey: " << clusterkey << std::endl;
              std::cout << "Clusterkey layer: " << unsigned(TrkrDefs::getLayer(clusterkey)) << std::endl;
              std::cout << "Clusterkey sector Id: " << unsigned(TrkrDefs::getPhiElement(clusterkey)) << std::endl;
              std::cout << "Clusterkey side: " << unsigned(TrkrDefs::getZElement(clusterkey)) << std::endl;
              std::cout << "m_cluster zSize: " << m_cluster->getZSize() << std::endl;
              ...
              //fill TPC and silicon seeds
                ...
                //calculate helix surface
                auto surface = tgeometry->maps().getSurface(clusterkey, m_cluster);
                auto geoLayer = tpcGeom->GetLayerCellGeom(layerbyte);
                auto radius = geoLayer->get_radius();
                std::cout << "radius: " << radius << std::endl;
                std::cout << "tpc_R: " << 1.0/abs(trackContainer2->tpc_seed_get_qOverR()) << std::endl;
                std::cout << "tpc_X0: " << trackContainer2->tpc_seed_get_X0() << std::endl;
                std::cout << "tpc_Y0: " << trackContainer2->tpc_seed_get_Y0() << std::endl;

                auto result = TrackFitUtils::circle_circle_intersection(radius, 1.0/abs(trackContainer2->tpc_seed_get_qOverR()), trackContainer2->tpc_seed_get_X0(), trackContainer2->tpc_seed_get_Y0());
                std::cout << "result(0): " << std::get<0>(result) << std::endl;
                std::cout << "result(1): " << std::get<1>(result) << std::endl;
                std::cout << "result(2): " << std::get<2>(result) << std::endl;
                std::cout << "result(3): " << std::get<3>(result) << std::endl;
                ...
                Acts::Vector3 globalIntersection;
                if(trackContainer2->get_is_tpc_helix_intersection_positive()){
                  //use xplus and yplus
                   globalIntersection[0] = std::get<0>(result);
                   globalIntersection[1] = std::get<1>(result); 
                   std::cout << "Using positive globalIntersection" << std::endl;
                }else{
                  //use xminus and yminus
                  globalIntersection[0] = std::get<2>(result);
                  globalIntersection[1] = std::get<3>(result);
                  std::cout << "Using negative globalIntersection" << std::endl;
                }
                globalIntersection[2] = radius * trackContainer2->tpc_seed_get_slope() + trackContainer2->tpc_seed_get_Z0();
                ...
                //residual is cluster - helix fit
                //cluster is residual + helix fit
                //calculate localXFromKey
                float residualX = m_compressor->decompressPhi(trackContainer2->getLocalXKeyResiduals(layer));
                float residualY = m_compressor->decompressZ(trackContainer2->getLocalYKeyResiduals(layer));

                std::cout << "residualX: " << residualX << std::endl;

                localX = residualX + localFromGlobalIntersectionNoTolerance(0);
                localY = residualY + yintersection;
