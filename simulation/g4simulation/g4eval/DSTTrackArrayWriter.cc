...
int DSTTrackArrayWriter::Init(PHCompositeNode* topNode )
{
  ...
  auto newArrayNode3 = new PHIODataNode<PHObject>( new SvtxTrackArrayContainer_v3, "SvtxTrackArrayContainer_v3","PHObject");
  evalNode->addNode(newArrayNode3);

  auto newArrayNode4 = new PHIODataNode<PHObject>( new SvtxTrackArrayContainer_v4, "SvtxTrackArrayContainer_v4","PHObject");
  evalNode->addNode(newArrayNode4);

  auto newArrayNode5 = new PHIODataNode<PHObject>( new SvtxTrackArrayContainer_v5, "SvtxTrackArrayContainer_v5","PHObject");
  evalNode->addNode(newArrayNode5);

  auto newArrayNode6 = new PHIODataNode<PHObject>( new SvtxTrackArrayContainer_v6, "SvtxTrackArrayContainer_v6","PHObject");
  evalNode->addNode(newArrayNode6);
  ...
  auto trackNode = findNode::getClass<SvtxTrackMap>(svtxNode, "ReducedTrackContainer");
  if (!trackNode)
  {
      auto newTrackNode = new PHIODataNode<PHObject>(new SvtxTrackMap_v2, "ReducedTrackContainer", "PHObject");
...
int DSTTrackArrayWriter::process_event(PHCompositeNode* topNode)
{
  ...
  if( m_track_array_container_v3) {
    m_track_array_container_v3->Reset();
  }

  if( m_track_array_container_v4) {
    m_track_array_container_v4->Reset();
  }

  if( m_track_array_container_v5) {
    m_track_array_container_v5->Reset();
    
    /*
    for(int i = 0; i<1000; i++){
      SvtxTrackArray_v5* dummy= new SvtxTrackArray_v5();
      m_track_array_container_v5->add_trackarray(i, dummy);
    }
    */
  }

  if( m_track_array_container_v6) {
    m_track_array_container_v6->Reset();
    
    
    //for(int i = 0; i<1000; i++){
    //  SvtxTrackArray_v6* dummy= new SvtxTrackArray_v6();
    //  m_track_array_container_v6->add_trackarray(i, dummy);
   // }
    
  }

  if( m_track_array_container_v7) {
    m_track_array_container_v7->Reset();
    
    /*
    for(int i = 0; i<1000; i++){
      SvtxTrackArray_v7* dummy= new SvtxTrackArray_v7();
  ...
  if( m_reduced_track_map) {
    m_reduced_track_map->Reset();
  }
  //evaluate_track_and_cluster_minimum_residual_compression();
  //evaluate_only_cluster_minimum_residual_compression();

  //evaluate_limited_track_and_cluster_minimum_residual_compression();
  
  //this is current function commented out
  //no_silicon_evaluate_limited_track_and_cluster_minimum_residual_compression();
...
int DSTTrackArrayWriter::load_nodes( PHCompositeNode* topNode )
{
  ...
  m_track_array_container_v3 = findNode::getClass<SvtxTrackArrayContainer_v3>(topNode, "SvtxTrackArrayContainer_v3");

  m_track_array_container_v4 = findNode::getClass<SvtxTrackArrayContainer_v4>(topNode, "SvtxTrackArrayContainer_v4");

  m_track_array_container_v5 = findNode::getClass<SvtxTrackArrayContainer_v5>(topNode, "SvtxTrackArrayContainer_v5");
...
void DSTTrackArrayWriter::fillNtpReducedCluster(float xKey, float yKey){
...
//_____________________________________________________________________
void DSTTrackArrayWriter::evaluate_track_and_cluster_minimum_residual_compression(){
//use this to create object that looks through both tracks and clusters and saves into new object
//make sure clusters exist
//std::cout << "start of check" << "\n";
//if(!(m_cluster_map&&m_hitsetcontainer&&m_container)) return;
//make sure tracks exist
if( !( m_track_map && m_cluster_map && m_track_array_container_v3 ) ) {
    return;
}
//std::cout << "after check" << "\n";
//m_track_array_container->Reset();
...
Int_t iTrk = 0;
  ...
  for( const auto& trackpair:*m_track_map )
  {
    ...
    */
    ...
    SvtxTrackArray_v3* trackContainer = new SvtxTrackArray_v3();
  ...
  TPCSeed->identify();
    for( auto key_iter = TPCSeed->begin_cluster_keys(); key_iter != TPCSeed->end_cluster_keys(); ++key_iter )
    {
      ...
      uint8_t totalAdcKey = (maxAdcKey << 4) + adcKey;   
      std::cout << "totalAdcKey: " << unsigned(totalAdcKey) << std::endl;
      trackContainer->setAdcInfoResiduals(tpcIndex, totalAdcKey);
 ...
 // }
  ...
  SiliconSeed->identify();
    for( auto key_iter = SiliconSeed->begin_cluster_keys(); key_iter != SiliconSeed->end_cluster_keys(); ++key_iter )
    {
      ...
      track_struct.clusters.push_back( cluster_struct );
      */
      m_track_array_container_v3->add_trackarray(iTrk, trackContainer);
      std::cout<< "track chisq: " << trackContainer->get_chisq() << std::endl;
      std::cout<< "container chisq: " << (m_track_array_container_v3->get_trackarray(iTrk))->get_chisq() << std::endl;
...
//don't save track or trackseed info


//_____________________________________________________________________
void DSTTrackArrayWriter::evaluate_only_cluster_minimum_residual_compression(){
//use this to create object that looks through both tracks and clusters and saves into new object
//make sure clusters exist
//std::cout << "start of check" << "\n";
//if(!(m_cluster_map&&m_hitsetcontainer&&m_container)) return;
...
//only save tracks with more clusters and also
//make new clustermap that stores only those clusters
...
//_____________________________________________________________________
void DSTTrackArrayWriter::evaluate_limited_track_and_cluster_minimum_residual_compression(){
//use this to create object that looks through both tracks and clusters and saves into new object
//make sure clusters exist
//std::cout << "start of check" << "\n";
//if(!(m_cluster_map&&m_hitsetcontainer&&m_container)) return;
//make sure tracks exist
if( !( m_track_map && m_cluster_map && m_track_array_container_v3&&m_reduced_cluster_map && m_reduced_track_map ) ) {
    return;
}
//std::cout << "after check" << "\n";
//m_track_array_container->Reset();
...
//make a version that doesn't save silicon seeds

//_____________________________________________________________________
void DSTTrackArrayWriter::no_silicon_evaluate_limited_track_and_cluster_minimum_residual_compression(){
//use this to create object that looks through both tracks and clusters and saves into new object
//make sure clusters exist
//std::cout << "start of check" << "\n";
//if(!(m_cluster_map&&m_hitsetcontainer&&m_container)) return;
//make sure tracks exist
if( !( m_track_map && m_cluster_map && m_track_array_container_v4&&m_reduced_cluster_map && m_reduced_track_map ) ) {
    return;
}
//std::cout << "after check" << "\n";
//m_track_array_container->Reset();
...
Int_t iTrk = 0;
  ...
  for( const auto& trackpair:*m_track_map )
  {
    ...
    */
    ...
    SvtxTrackArray_v4* trackContainer = new SvtxTrackArray_v4();
  ...
  */
      ...
      track_struct.clusters.push_back( cluster_struct );
      */
      m_track_array_container_v4->add_trackarray(iTrk, trackContainer);
      std::cout<< "track chisq: " << trackContainer->get_chisq() << std::endl;
      std::cout<< "container chisq: " << (m_track_array_container_v4->get_trackarray(iTrk))->get_chisq() << std::endl;
      ++iTrk;
  }
...
//_____________________________________________________________________
void DSTTrackArrayWriter::no_silicon_evaluate_limited_trackv5_and_cluster_minimum_residual_compression(){
//use this to create object that looks through both tracks and clusters and saves into new object
//make sure clusters exist
//std::cout << "start of check" << "\n";
//if(!(m_cluster_map&&m_hitsetcontainer&&m_container)) return;
//make sure tracks exist
if( !( m_track_map && m_cluster_map && m_track_array_container_v5&&m_reduced_cluster_map && m_reduced_track_map ) ) {
    return;
}
//std::cout << "after check" << "\n";
//m_track_array_container->Reset();
...
Int_t iTrk = 0;
  ...
  for( const auto& trackpair:*m_track_map )
  {
    ...
    */
    ...
    SvtxTrackArray_v5* trackContainer = new SvtxTrackArray_v5();
  ...
  */
      ...
      track_struct.clusters.push_back( cluster_struct );
      */
      m_track_array_container_v5->add_trackarray(iTrk, trackContainer);
      std::cout<< "track chisq: " << trackContainer->get_chisq() << std::endl;
      std::cout<< "container chisq: " << (m_track_array_container_v5->get_trackarray(iTrk))->get_chisq() << std::endl;
      ++iTrk;
  }
...
//_____________________________________________________________________
void DSTTrackArrayWriter::no_silicon_evaluate_limited_trackv6_and_cluster_minimum_residual_compression(){
//use this to create object that looks through both tracks and clusters and saves into new object
//make sure clusters exist
//std::cout << "start of check" << "\n";
//if(!(m_cluster_map&&m_hitsetcontainer&&m_container)) return;
//make sure tracks exist
if( !( m_track_map && m_cluster_map && m_track_array_container_v6&&m_reduced_cluster_map && m_reduced_track_map ) ) {
    return;
}
//std::cout << "after check" << "\n";
//m_track_array_container->Reset();
...
Int_t iTrk = 0;
  ...
  for( const auto& trackpair:*m_track_map )
  {
    ...
    */
    ...
    SvtxTrackArray_v6* trackContainer = new SvtxTrackArray_v6();
  ...
  */
      ...
      track_struct.clusters.push_back( cluster_struct );
      */
      m_track_array_container_v6->add_trackarray(iTrk, trackContainer);
      //std::cout<< "track chisq: " << trackContainer->get_chisq() << std::endl;
...
//make a v7 version that only saves the cluster information


//_____________________________________________________________________
void DSTTrackArrayWriter::no_silicon_evaluate_limited_trackv7_and_cluster_minimum_residual_compression(){
//use this to create object that looks through both tracks and clusters and saves into new object
//make sure clusters exist
//std::cout << "start of check" << "\n";
//if(!(m_cluster_map&&m_hitsetcontainer&&m_container)) return;
//make sure tracks exist
if( !( m_track_map && m_cluster_map && m_track_array_container_v7&&m_reduced_cluster_map && m_reduced_track_map ) ) {
    return;
}
//std::cout << "after check" << "\n";
//m_track_array_container->Reset();
...
Int_t iTrk = 0;
  ...
  for( const auto& trackpair:*m_track_map )
  {
    ...
    */
    ...
    SvtxTrackArray_v7* trackContainer = new SvtxTrackArray_v7();
