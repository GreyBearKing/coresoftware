#ifndef SVTXTRACKARRAYCONTAINERV9_H
#define SVTXTRACKARRAYCONTAINERV9_H
...
class SvtxTrackArrayContainer_v9 : public SvtxTrackArrayContainer
{
    public:
 
    SvtxTrackArrayContainer_v9();
    ~SvtxTrackArrayContainer_v9() override;
    ...
    void add_trackarray(int pos, SvtxTrackArray* trackinfo) override{
      new((*_clones)[pos]) SvtxTrackArray_v9;
      trackinfo->identify();
      std::cout << "Right before ConstructedAt9" << std::endl;
      SvtxTrackArray_v9 *info = (SvtxTrackArray_v9*)_clones->ConstructedAt(pos);
      std::cout << "Right before copyFrom9" << std::endl;
      info->CopyFrom(trackinfo);
      info->identify();
    ...
    private:
    ClassDefOverride(SvtxTrackArrayContainer_v9, 1);
};
