...
*/
SvtxTrackArray_v5::SvtxTrackArray_v5(){
// always include the pca point
  //_states.insert( std::make_pair(0, new SvtxTrackState_v1(0)));
}

SvtxTrackArray_v5::SvtxTrackArray_v5(const SvtxTrackArray& source)
{ SvtxTrackArray_v5::CopyFrom( source ); }

// have to suppress uninitMenberVar from cppcheck since it triggers many false positive
// cppcheck-suppress uninitMemberVar
SvtxTrackArray_v5::SvtxTrackArray_v5(const SvtxTrackArray_v5& source)
{ SvtxTrackArray_v5::CopyFrom( source ); }

SvtxTrackArray_v5& SvtxTrackArray_v5::operator=(const SvtxTrackArray_v5& source)
{ CopyFrom( source ); return *this; }

//IMPROVE
void SvtxTrackArray_v5::CopyFrom( const SvtxTrackArray& source )
{
  ...
  */
  ...
  for(int i = 0; i<48; i++){
    //check if source has valid cluster
        ...
        setAdcInfoResiduals(i, source.getAdcInfoResiduals(i));
...
void SvtxTrackArray_v5::identify(std::ostream& os) const
{
    os << "SvtxTrackArray_v5 Object " << std::endl;
...
*/
int SvtxTrackArray_v5::isValid() const
{
  return 1;
}
...
/*
void SvtxTrackArray_v1::addTrack(SvtxTrack_v5 &track, unsigned int key, int index){
    //SvtxTrack_v5* tmpTrack = TrackArray->ConstructedAt(index);
    TrackArray[index]= new SvtxTrack_v5(track);
...
SvtxTrack_v5* SvtxTrackArray_v1::findTrack(unsigned int key){
