...
*/
SvtxTrackArray_v10::SvtxTrackArray_v10(){
// always include the pca point
  //_states.insert( std::make_pair(0, new SvtxTrackState_v1(0)));
}

SvtxTrackArray_v10::SvtxTrackArray_v10(const SvtxTrackArray& source)
{ SvtxTrackArray_v10::CopyFrom( source ); }

// have to suppress uninitMenberVar from cppcheck since it triggers many false positive
// cppcheck-suppress uninitMemberVar
SvtxTrackArray_v10::SvtxTrackArray_v10(const SvtxTrackArray_v10& source)
{ SvtxTrackArray_v10::CopyFrom( source ); }

SvtxTrackArray_v10& SvtxTrackArray_v10::operator=(const SvtxTrackArray_v10& source)
{ CopyFrom( source ); return *this; }

//IMPROVE
void SvtxTrackArray_v10::CopyFrom( const SvtxTrackArray& source )
{
  ...
  */
  ...
  for(int i = 0; i<48; i++){
    //check if source has valid cluster
        ...
        setLocalXInt(i, source.getLocalXInt(i));
        setLocalYInt(i, source.getLocalYInt(i));
...
void SvtxTrackArray_v10::identify(std::ostream& os) const
{
    os << "SvtxTrackArray_v10 Object " << std::endl;
...
*/
int SvtxTrackArray_v10::isValid() const
{
  return 1;
}
...
/*
void SvtxTrackArray_v1::addTrack(SvtxTrack_v10 &track, unsigned int key, int index){
    //SvtxTrack_v10* tmpTrack = TrackArray->ConstructedAt(index);
    TrackArray[index]= new SvtxTrack_v10(track);
...
SvtxTrack_v10* SvtxTrackArray_v1::findTrack(unsigned int key){
